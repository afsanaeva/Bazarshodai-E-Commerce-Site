...
class Cart extends DB {
  ...
  // details() : get details of items in cart
    ...
    // Get products1 in cart
    $sql = "SELECT * FROM `products1` WHERE `product_id` IN (";
  ...
  // PARAM $id : order ID
    ...
    $order['items'] = $this->fetch(
      "SELECT * FROM `orders_items` LEFT JOIN `products1` USING (`product_id`) WHERE `orders_items`.order_id=?", 
      [$id], "product_id"
    );
    return $order;
  }
