...
require __DIR__ . DIRECTORY_SEPARATOR . "lib" . DIRECTORY_SEPARATOR . "2a-config1.php";
switch ($_POST['req']) {
  /* [INVALID REQUEST] */
  default:
  ...
  case "show":
    // Fetch products
    require PATH_LIB . "2b-lib-db1.php";
    require PATH_LIB . "4c-lib-cart1.php";
    $cartLib = new Cart();
    $products1 = $cartLib->details();
    ...
    <table id="cart-table">
      ...
      if (count($_SESSION['cart'])>0) {
      foreach ($_SESSION['cart'] as $id => $qty) {
        $sub = $qty * $products1[$id]['product_price'];
        $total += $sub; ?>
      <tr>
        <td>
        ...
        <td><?= $products1[$id]['product_name'] ?></td>
    ...
    <form id="cart-checkout" onsubmit="return cart.checkout();">
      <label>Name</label>
      <input type="text" id="co_name" required value="Syed Mukibur Rahman"/>
      <label>Email</label>
      <input type="email" id="co_email" required value="mukibrahman0@gmail.com"/>
